...
EPOCHS = 50
BATCH_SIZE = 128
MODEL_DIR = "models"; os.makedirs(MODEL_DIR, exist_ok=True); os.makedirs("results", exist_ok=True)
...
def train(ds_name):
    ...
    if not os.path.exists(path):
        print(f"⚠ UYARI: Preprocess edilmiş veri bulunamadı: {path}. {ds_name} atlanıyor."); return
    ...
    save_path = f"{MODEL_DIR}/{ds_name}_vae.pth"
    torch.save(model.state_dict(), save_path)
    print(f"Tamamlandı ({duration:.4f} saat) → {save_path}")

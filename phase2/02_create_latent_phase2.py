...
import os
LATENT_DIR = "latent_dataset"; os.makedirs(LATENT_DIR, exist_ok=True)
...
def process(ds_name):
    ...
    if not os.path.exists(data_path):
        print(f"⚠ UYARI: Preprocess edilmiş veri bulunamadı: {data_path}"); return
    if not os.path.exists(model_path):
        print(f"⚠ UYARI: VAE modeli bulunamadı: {model_path}"); return
    data = np.load(data_path).astype(np.float32)
    vae = VAE(data.shape[1])
    vae.load_state_dict(torch.load(model_path, map_location="cpu"), strict=False)
    vae.eval()
    with torch.no_grad():
